...
const mockConfig = {
    ...
    update: async (key, val) => console.log(`[Config Update] ${key} = ${val}`)
};

const mockPlatform = {
    getAppName: () => "KongAgentTest",
    getAppReferer: () => "test",
    openExternal: async (url) => console.log(`[Open URL] ${url}`),
    getStoragePath: () => "./tmp",
    showInformationMessage: (msg) => console.log(`[Info] ${msg}`),
    showErrorMessage: (msg) => console.error(`[Error] ${msg}`),
    openFileInEditor: async (path) => console.log(`[Open File] ${path}`)
};


const toolManager = new ToolManager(mockConfig, mockPlatform);

// 4. Instantiate the model explicitly (OpenRouter compatible)
const model = new ChatOpenAI({
    modelName: "z-ai/glm-4.7-flash",
    apiKey: process.env.OPENAI_API_KEY || "", // Use env var or placeholder
    configuration: {
